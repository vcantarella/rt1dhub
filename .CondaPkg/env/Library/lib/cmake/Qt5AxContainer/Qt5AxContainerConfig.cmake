...
set(Qt5AxContainer_VERSION_STRING 5.15.8)
...
function(_qt5_AxContainer_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    set(_qt5_install_libs "${_qt5AxContainer_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5AxContainer_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5AxContainer_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
...
macro(_populate_AxContainer_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set_property(TARGET Qt5::AxContainer APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 "${_deps}"
    )
    ...
    set(_static_link_flags "${_Qt5AxContainer_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::AxContainer APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::AxContainer)
    ...
    foreach(_module_dep ${_Qt5AxContainer_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.8 ${_Qt5AxContainer_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_AxContainer_process_prl_file(
            "${_qt5AxContainer_install_prefix}/lib/Qt5AxContainer_conda.prl" RELEASE
    ...
    set_property(TARGET Qt5::AxContainer
    ...
    set_property(TARGET Qt5::AxContainer PROPERTY INTERFACE_QT_PLUGIN_TYPES "")
    ...
    if (_Qt5AxContainer_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::AxContainerPrivate)
            add_library(Qt::AxContainerPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::AxContainerPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::AxContainerPrivate"
            )
        endif()
    endif()

    _populate_AxContainer_target_properties(RELEASE "Qt5AxContainer_conda.lib" "" FALSE)

    if (EXISTS "${_qt5AxContainer_install_prefix}/lib/Qt5AxContainer_condad.lib" )
        _populate_AxContainer_target_properties(DEBUG "Qt5AxContainer_condad.lib" "" FALSE)
    endif()
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5AxContainer_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5AxContainer_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5AxContainer_*.cmake")
    endif()
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::AxContainer AND NOT TARGET Qt::AxContainer)
    add_library(Qt::AxContainer INTERFACE IMPORTED)
    set_target_properties(Qt::AxContainer PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::AxContainer"
    )
endif()
