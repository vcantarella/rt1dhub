...
set(Qt5Charts_VERSION_STRING 5.15.8)
...
macro(_populate_Charts_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set_property(TARGET Qt5::Charts APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 "${_deps}"
    )
...
if (NOT TARGET Qt5::Charts)
    ...
    set(_Qt5Charts_OWN_INCLUDE_DIRS "${_qt5Charts_install_prefix}/include/qt/" "${_qt5Charts_install_prefix}/include/qt/QtCharts")
    set(Qt5Charts_PRIVATE_INCLUDE_DIRS
        "${_qt5Charts_install_prefix}/include/qt/QtCharts/5.15.8"
        "${_qt5Charts_install_prefix}/include/qt/QtCharts/5.15.8/QtCharts"
    )
    ...
    foreach(_module_dep ${_Qt5Charts_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.8 ${_Qt5Charts_FIND_VERSION_EXACT}
    ...
    set_property(TARGET Qt5::Charts
    ...
    set_property(TARGET Qt5::Charts PROPERTY INTERFACE_QT_PLUGIN_TYPES "")
    ...
    if (_Qt5Charts_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::ChartsPrivate)
            add_library(Qt::ChartsPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::ChartsPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::ChartsPrivate"
            )
        endif()
    endif()

    _populate_Charts_target_properties(RELEASE "Qt5Charts_conda.dll" "Qt5Charts_conda.lib" FALSE)

    if (EXISTS
        "${_qt5Charts_install_prefix}/bin/Qt5Charts_condad.dll"
      AND EXISTS
        "${_qt5Charts_install_prefix}/lib/Qt5Charts_condad.lib" )
        _populate_Charts_target_properties(DEBUG "Qt5Charts_condad.dll" "Qt5Charts_condad.lib" FALSE)
    endif()
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5Charts_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5Charts_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5Charts_*.cmake")
    endif()
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::Charts AND NOT TARGET Qt::Charts)
    add_library(Qt::Charts INTERFACE IMPORTED)
    set_target_properties(Qt::Charts PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::Charts"
    )
endif()
