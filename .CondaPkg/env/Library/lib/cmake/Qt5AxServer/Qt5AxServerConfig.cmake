...
set(Qt5AxServer_VERSION_STRING 5.15.8)
...
function(_qt5_AxServer_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5AxServer_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5AxServer_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5AxServer_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5AxServer_${Configuration}_${_lib}_PATH)
                    if(_Qt5AxServer_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5AxServer_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_AxServer_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_deps
        ${_Qt5AxServer_LIB_DEPENDENCIES}
    )
    set(_static_deps
        ${_Qt5AxServer_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set_property(TARGET Qt5::AxServer APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 "${_deps}"
    )
    ...
    set(_static_link_flags "${_Qt5AxServer_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::AxServer APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::AxServer)
    ...
    foreach(_module_dep ${_Qt5AxServer_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.8 ${_Qt5AxServer_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_AxServer_process_prl_file(
            "${_qt5AxServer_install_prefix}/lib/Qt5AxServer_conda.prl" RELEASE
            _Qt5AxServer_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5AxServer_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    ...
    set_property(TARGET Qt5::AxServer
    ...
    set_property(TARGET Qt5::AxServer PROPERTY INTERFACE_QT_PLUGIN_TYPES "")
    ...
    if (_Qt5AxServer_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::AxServerPrivate)
            add_library(Qt::AxServerPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::AxServerPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::AxServerPrivate"
            )
        endif()
    endif()

    _populate_AxServer_target_properties(RELEASE "Qt5AxServer_conda.lib" "" FALSE)

    if (EXISTS "${_qt5AxServer_install_prefix}/lib/Qt5AxServer_condad.lib" )
        _populate_AxServer_target_properties(DEBUG "Qt5AxServer_condad.lib" "" FALSE)
    endif()
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5AxServer_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5AxServer_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5AxServer_*.cmake")
    endif()
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::AxServer AND NOT TARGET Qt::AxServer)
    add_library(Qt::AxServer INTERFACE IMPORTED)
    set_target_properties(Qt::AxServer PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::AxServer"
    )
endif()
