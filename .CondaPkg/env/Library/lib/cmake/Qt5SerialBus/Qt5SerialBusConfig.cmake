...
set(Qt5SerialBus_VERSION_STRING 5.15.8)
...
if (NOT TARGET Qt5::SerialBus)
    ...
    set(_Qt5SerialBus_OWN_INCLUDE_DIRS "${_qt5SerialBus_install_prefix}/include/qt/" "${_qt5SerialBus_install_prefix}/include/qt/QtSerialBus")
    set(Qt5SerialBus_PRIVATE_INCLUDE_DIRS
        "${_qt5SerialBus_install_prefix}/include/qt/QtSerialBus/5.15.8"
        "${_qt5SerialBus_install_prefix}/include/qt/QtSerialBus/5.15.8/QtSerialBus"
    )
    ...
    foreach(_module_dep ${_Qt5SerialBus_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.8 ${_Qt5SerialBus_FIND_VERSION_EXACT}
    ...
    set_property(TARGET Qt5::SerialBus
    ...
    if (_Qt5SerialBus_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::SerialBusPrivate)
            add_library(Qt::SerialBusPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::SerialBusPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::SerialBusPrivate"
            )
        endif()
    endif()

    _populate_SerialBus_target_properties(RELEASE "Qt5SerialBus_conda.dll" "Qt5SerialBus_conda.lib" FALSE)

    if (EXISTS
        "${_qt5SerialBus_install_prefix}/bin/Qt5SerialBus_condad.dll"
      AND EXISTS
        "${_qt5SerialBus_install_prefix}/lib/Qt5SerialBus_condad.lib" )
        _populate_SerialBus_target_properties(DEBUG "Qt5SerialBus_condad.dll" "Qt5SerialBus_condad.lib" FALSE)
    endif()
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5SerialBus_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5SerialBus_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5SerialBus_*.cmake")
    endif()
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::SerialBus AND NOT TARGET Qt::SerialBus)
    add_library(Qt::SerialBus INTERFACE IMPORTED)
    set_target_properties(Qt::SerialBus PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::SerialBus"
    )
endif()
