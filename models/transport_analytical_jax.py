...
@jax.jit
def constant_injection(x: jnp.ndarray,
                       t: jnp.ndarray,
                       c0: jnp.float64,
                       c_in: jnp.float64, 
                       v: jnp.float64,
                       Dl: jnp.float64) -> jnp.ndarray:
    def compute_cr(i):
        return c_in + (c0 - c_in) / 2 * (
            jscipy.special.erfc((x[i] - v * t) / (2 * jnp.sqrt(Dl * t))) +
            jnp.exp(v * x[i] / Dl) * jscipy.special.erfc((x[i] + v * t) / (2 * jnp.sqrt(Dl * t)))
        )
    cr = jax.vmap(compute_cr)(jnp.arange(x.shape[0]))
    return cr.T

@jax.jit
def pulse_injection(x: jnp.ndarray,
                    t: jnp.ndarray,
                    c0: jnp.float64,
                    c_in: jnp.float64, 
                    v: jnp.float64,
                    Dl: jnp.float64,
                    t_pulse: jnp.float64) -> jnp.ndarray:
    ...
    def compute_cr(i):
        ...
        co = - (c0 - c_in) / 2 * (
            jscipy.special.erfc((x[i] - v * (t - tau)) / (2 * jnp.sqrt(Dl * (t - tau)))) +
            jnp.exp(v * x[i] / Dl) * jscipy.special.erfc((x[i] + v * (t - tau)) / (2 * jnp.sqrt(Dl * (t - tau))))
        )
        return c_init + co
...
if __name__ == "__main__":
    ...
    cr_pulse = jnp.zeros((t.shape[0], x.shape[0]))
